...
/*$('#simpan_data').click(function(event) {
	var base 		= $('#base').val()
	var pangkalan 	= $('select[name="pangkalan"]').val()
	var no_gudep 	= $('input[name="gudep"]').val()
	var ambalan 	= $('select[name="ambalan"]').val()
	...
	else
	{
		...
		.fail(function() {
			Swal.fire({
				title: 'Maaf,',
				text: 'Gagal Input, Silahkan Ulangi Atau Gunakan Koneksi Yang Baik',
				icon: 'error',
				confirmButtonColor: '#3085d6',
				confirmButtonText: 'Ok'
			}).then((result) => {
